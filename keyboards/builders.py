...
def main_menu():
    ...
    builder.adjust(2, 2, 1) # Чистое меню без лишних кнопок "Назад"
...
def clients_menu():
    ...
    builder.button(text="⬅️ Главное меню") # Из подменю вернуться на главную — это ок
    builder.adjust(2, 1)
    return builder.as_markup(resize_keyboard=True)
...
def client_type_selection():
    ...
    builder.button(text="❌ Отмена") # Кнопка возврата, если передумали
    builder.adjust(3, 1)
    return builder.as_markup(resize_keyboard=True)
...
def date_selection():
    ...
    builder.button(text="❌ Отмена") # Добавили отмену в выбор даты часов
...
def month_selection():
    ...
    builder.button(text="❌ Отмена") # Добавили отмену в выбор месяца сверки
    builder.adjust(2, 1)
    return builder.as_markup(resize_keyboard=True)
